...
_order_counter = _load_counter() # Завантажуємо при старті

# --- Імена постачальників ---
SUPPLIER_NAMES: Dict[str, str] = {
    "landliz": "Landliz Drop", "unknown": "Невідомий",
    # TODO: Додавати інших постачальників (можливо, завантажувати з БД/конфігу)
}
...
def generate_order_filename(order_id: str, customer_name: str | None) -> str:
    ...
    # Замінюємо потенційно небезпечні символи в ініціалах
    safe_initials = "".join(c if c.isalnum() or c in '._-' else '_' for c in initials)
    return f"{order_id}_{safe_initials}_{date_str}.txt"
