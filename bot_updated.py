...
async def rewrite_text_with_ai(text_to_rewrite: str, product_name: str) -> str:
    """
    Відправляє текст до Google Gemini API для рерайтингу, повертає новий текст.
    Використовує офіційний Python SDK 'google-genai'.
    """
    ...
    try:
        # Модель з відключеним "thinking" для швидкості та економії
        model = genai.GenerativeModel(
            ...
            system_instruction=(
                ...
                "Використовуay доречні емодзі (напр. 🛡️, 💪, 🎯, 🔥). "
...
def gdrive_find_or_create_folder(folder_name: str, parent_folder_id: str):
    """
    Шукає папку за назвою всередині батьківської папки.
    Якщо не знаходить - створює її. Повертає ID папки.
    """
    if not GDRIVE_SERVICE:
        return None
    try:
        # 1. Спробувати знайти папку
        query = f"'{parent_folder_id}' in parents and name = '{folder_name}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
        response = GDRIVE_SERVICE.files().list(q=query, spaces='drive', fields='files(id, name)').execute()
        files = response.get('files', [])
        
        if files:
            logger.info(f"GDrive: Папку '{folder_name}' знайдено, ID: {files[0].get('id')}")
            return files[0].get('id')

        # 2. Якщо не знайдено - створити
        ...
        logger.info(f"GDrive: Папку '{folder_name}' створено, ID: {folder_id}")
        return folder_id
...
async def main():
    ...
    # --- ДОДАНО: КОНФІГУРАЦІЯ GEMINI API ---
    try:
        gemini_api_key = os.getenv("GEMINI_API_KEY")
        if gemini_api_key:
            genai.configure(api_key=gemini_api_key)
            logger.info("✅ Google Gemini API сконфігуровано успішно.")
        else:
            logger.warning("⚠️ GEMINI_API_KEY не знайдено в .env. Рерайтинг тексту буде недоступний.")
    except Exception as e:
        logger.error(f"❌ Помилка конфігурації Gemini API: {e}")
