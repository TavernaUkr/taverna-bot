    ...
    try:
        ...
        for _, elem in it:
            if elem.tag == 'offer':
                ...
                vendor_code = (elem.find('vendorCode').text or "").strip()

                if not offer_id or not name or not drop_price:
                    elem.clear()
                    continue
                
                description = (elem.find('description').text or "").strip()
...
def find_product_by_sku(raw: str) -> Optional[List[dict]]:
    ...
    if exact_matches:
        for p in exact_matches: p['match_type'] = 'exact'
        logger.debug(f"Lookup EXACT success for SKU='{raw}': Found {len(exact_matches)} products.")
        ...
        if raw_norm in key:
            for p in products:
                if p['offer_id'] not in processed_offers:
                    p['match_type'] = 'suggestion'
                    suggestion_matches.append(p)
    ...
    while True:
        try:
            ...
            for _ in range(20):
                ...
                if messages and f"{messages[0].chat_id}_{messages[0].id}" not in POSTED_IDS:
